    ...
    <main className="grid min-h-screen grid-cols-[minmax(0,1.05fr)_minmax(420px,.95fr)] bg-bg lg:block">
      <section className="relative overflow-hidden bg-[linear-gradient(135deg,#0f172a_0%,#052e2b_55%,#111827_100%)] p-10 text-white before:absolute before:-right-32 before:-top-32 before:h-96 before:w-96 before:rounded-full before:bg-primary-lighter/30 before:blur-3xl after:absolute after:-bottom-28 after:-left-24 after:h-80 after:w-80 after:rounded-full after:bg-teal-500/20 after:blur-3xl max-lg:hidden">
        <div className="relative z-10 flex h-full flex-col justify-between gap-10">
          <div className="flex items-center gap-4 [&_strong]:block [&_strong]:text-2xl [&_strong]:font-black [&_p]:m-0 [&_p]:text-sm [&_p]:font-semibold [&_p]:text-slate-500">
      ...
      <section className="flex items-center justify-center bg-surface-alt p-6 lg:p-12">
        <div className="mb-6 hidden items-center gap-3 lg:flex [&_strong]:block [&_strong]:text-xl [&_strong]:font-black [&_strong]:text-slate-900 [&_small]:block [&_small]:text-xs [&_small]:font-semibold [&_small]:text-slate-500">
        ...
        <div className="w-full max-w-md rounded-3xl border border-border bg-white p-8 shadow-float">
          <div className="mb-6 [&_h2]:mb-1 [&_h2]:text-2xl [&_h2]:font-black [&_h2]:text-slate-900 [&_p]:m-0 [&_p]:text-sm [&_p]:font-semibold [&_p]:text-slate-500">
          ...
          <form onSubmit={isSetup ? onSubmitSetup : onSubmitLogin} className="grid gap-4">
            {isSetup && (
              <label className="grid gap-2 text-sm font-black text-ink-soft">Nama Owner
                <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-500"><User size={20} /><input className="w-full rounded-2xl border border-border bg-white pl-12 pr-4 py-3.5 text-base text-ink font-semibold transition-colors duration-150 focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/15" autoFocus value={setupForm.name} onChange={(e) => onSetupFormChange({ ...setupForm, name: e.target.value })} placeholder="Nama pemilik toko" /></div>
              </label>
            )}
            <label className="grid gap-2 text-sm font-black text-ink-soft">Username
              <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-500"><User size={20} /><input className="w-full rounded-2xl border border-border bg-white pl-12 pr-4 py-3.5 text-base text-ink font-semibold transition-colors duration-150 focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/15" autoFocus={!isSetup} value={usernameValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, username: e.target.value }) : onLoginFormChange({ ...loginForm, username: e.target.value })} placeholder="Masukkan username" /></div>
            </label>
            <label className="grid gap-2 text-sm font-black text-ink-soft">Password
              <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-500"><Lock size={20} /><input className="w-full rounded-2xl border border-border bg-white pl-12 pr-12 py-3.5 text-base text-ink font-semibold transition-colors duration-150 focus:border-primary focus:outline-none focus:ring-4 focus:ring-primary/15" type={showPassword ? "text" : "password"} value={passwordValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, password: e.target.value }) : onLoginFormChange({ ...loginForm, password: e.target.value })} placeholder="Masukkan password" /><button type="button" className="absolute right-2 grid h-9 w-9 place-items-center rounded-xl border-0 bg-transparent p-0 text-slate-500 shadow-none hover:translate-y-0 hover:bg-slate-100 hover:text-slate-600 hover:shadow-none" onClick={onTogglePassword} aria-label={showPassword ? "Sembunyikan password" : "Lihat password"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>
            </label>
