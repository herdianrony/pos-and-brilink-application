    ...
    <main className="login-shell">
      ...
      <section className="login-form-panel">
        ...
        <div className="login-card">
          ...
          <form onSubmit={isSetup ? onSubmitSetup : onSubmitLogin} className="login-form">
            {isSetup && (
              <label>Nama Owner
                <div className="login-input-wrap"><User size={20} /><input autoFocus value={setupForm.name} onChange={(e) => onSetupFormChange({ ...setupForm, name: e.target.value })} placeholder="Nama pemilik toko" /></div>
              </label>
            )}
            <label>Username
              <div className="login-input-wrap"><User size={20} /><input autoFocus={!isSetup} value={usernameValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, username: e.target.value }) : onLoginFormChange({ ...loginForm, username: e.target.value })} placeholder="Masukkan username" /></div>
            </label>
            <label>Password
              <div className="login-input-wrap"><Lock size={20} /><input type={showPassword ? "text" : "password"} value={passwordValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, password: e.target.value }) : onLoginFormChange({ ...loginForm, password: e.target.value })} placeholder="Masukkan password" /><button type="button" className="input-icon-button" onClick={onTogglePassword} aria-label={showPassword ? "Sembunyikan password" : "Lihat password"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>
            </label>
