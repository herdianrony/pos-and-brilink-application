          ...
          <form onSubmit={isSetup ? onSubmitSetup : onSubmitLogin} className="grid gap-4">
            {isSetup && (
              <label className="grid gap-2 text-[13px] font-black text-slate-600">Nama Owner
                <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-400 [&_input]:w-full [&_input]:rounded-2xl [&_input]:border-2 [&_input]:border-slate-200 [&_input]:bg-slate-50/60 [&_input]:py-3.5 [&_input]:pl-12 [&_input]:pr-4 [&_input]:font-semibold [&_input]:focus:bg-white"><User size={20} /><input className="w-full rounded-2xl border border-slate-200 bg-white px-3.5 py-3 text-[15px] text-slate-900 transition-all duration-150 focus:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-500/15" autoFocus value={setupForm.name} onChange={(e) => onSetupFormChange({ ...setupForm, name: e.target.value })} placeholder="Nama pemilik toko" /></div>
              </label>
            )}
            <label className="grid gap-2 text-[13px] font-black text-slate-600">Username
              <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-400 [&_input]:w-full [&_input]:rounded-2xl [&_input]:border-2 [&_input]:border-slate-200 [&_input]:bg-slate-50/60 [&_input]:py-3.5 [&_input]:pl-12 [&_input]:pr-4 [&_input]:font-semibold [&_input]:focus:bg-white"><User size={20} /><input className="w-full rounded-2xl border border-slate-200 bg-white px-3.5 py-3 text-[15px] text-slate-900 transition-all duration-150 focus:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-500/15" autoFocus={!isSetup} value={usernameValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, username: e.target.value }) : onLoginFormChange({ ...loginForm, username: e.target.value })} placeholder="Masukkan username" /></div>
            </label>
            <label className="grid gap-2 text-[13px] font-black text-slate-600">Password
              <div className="relative flex items-center [&_svg]:absolute [&_svg]:left-4 [&_svg]:text-slate-400 [&_input]:w-full [&_input]:rounded-2xl [&_input]:border-2 [&_input]:border-slate-200 [&_input]:bg-slate-50/60 [&_input]:py-3.5 [&_input]:pl-12 [&_input]:pr-4 [&_input]:font-semibold [&_input]:focus:bg-white"><Lock size={20} /><input className="w-full rounded-2xl border border-slate-200 bg-white px-3.5 py-3 text-[15px] text-slate-900 transition-all duration-150 focus:border-emerald-500 focus:outline-none focus:ring-4 focus:ring-emerald-500/15" type={showPassword ? "text" : "password"} value={passwordValue} onChange={(e) => isSetup ? onSetupFormChange({ ...setupForm, password: e.target.value }) : onLoginFormChange({ ...loginForm, password: e.target.value })} placeholder="Masukkan password" /><button type="button" className="absolute right-2 grid h-9 w-9 place-items-center rounded-xl border-0 bg-transparent p-0 text-slate-400 shadow-none hover:translate-y-0 hover:bg-slate-100 hover:text-slate-600 hover:shadow-none" onClick={onTogglePassword} aria-label={showPassword ? "Sembunyikan password" : "Lihat password"}>{showPassword ? <EyeOff size={18} /> : <Eye size={18} />}</button></div>
            </label>
