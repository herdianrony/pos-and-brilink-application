  ...
  return (
    <>
      <div className="page-title">
        ...
        <div className="page-actions"><button className="secondary" onClick={onHoldCart} disabled={!cart.length}>Hold</button><button className="danger" onClick={onClearCart} disabled={!cart.length}>Kosongkan</button></div>
      </div>
      <div className="pos-shell">
        <section className="pos-catalog card">
          ...
          <div className="pos-product-grid">
            {products.length === 0 ? <div className="empty-state"><strong>Produk tidak ditemukan</strong><span>Tambahkan produk baru atau ubah kata kunci pencarian.</span></div> : products.map((product) => (
              <div key={product.id} className="pos-product-card">
                ...
                <div className="product-actions">
                  <button onClick={() => onAddToCart(product)} disabled={product.stock <= 0}>Tambah</button>
                </div>
              </div>
            ))}
          </div>
