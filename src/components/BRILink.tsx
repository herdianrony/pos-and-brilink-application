...
import {
  BankIcon,
  isBankIcon as isSupportedBankIcon,
} from "@/components/BankIcon";
...
import type {
  Account,
  AgentService as Service,
  ServiceCategory as ServiceCat,
} from "@/types/models";
...
export default function BRILink() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      setCats(c);
      setAccounts(a);
      const bankAccs = a.filter(
        (acc: Account) => acc.code !== "cash" && acc.isActive !== false,
      );
  ...
  const filtered = services.filter((s) => {
    if (catFilter !== "all" && s.categoryId?.toString() !== catFilter)
      return false;
  ...
  const bankBalanceAfter = selectedBank
    ? bankBalanceBefore + bankFlow.bankDelta
    : bankBalanceBefore;
  ...
  const denominationTotal = useMemo(() => {
    return Object.entries(denomination).reduce(
  ...
  const hasEnoughBankBalance =
    !bankNeedsBalance || bankBalance >= bankImpactAmount;
  const cashNeedsBalance = sel?.cashEffect === "out";
  const hasEnoughCashBalance =
    !cashNeedsBalance || cashBalanceBefore >= cashFlow.cashDispensed;
  const cashShortfall =
    cashNeedsBalance && cashFlow.cashDispensed > cashBalanceBefore
      ? cashFlow.cashDispensed - cashBalanceBefore
      : 0;
  const hasRequiredBankAccount = sel?.bankEffect === "none" || !!selectedBank;
  const canSubmit =
    (flowConfig?.requiresNominal === false || nominalAmount > 0) &&
    hasRequiredBankAccount &&
    hasEnoughBankBalance &&
    hasEnoughCashBalance;
  ...
  function toggleFavorite(serviceId: number) {
    setFavorites((prev) => {
      const updated = prev.includes(serviceId)
        ...
        : [...prev, serviceId];
      try {
        localStorage.setItem("brilink_favorites", JSON.stringify(updated));
      } catch {}
      return updated;
    });
  }
    ...
    try {
      const res = await fetch("/api/transactions", {
        ...
        body: JSON.stringify({
          ...
          bankAccountId: form.selectedBankId
            ? parseInt(form.selectedBankId)
            : null,
      ...
      (async () => {
        try {
          if (typeof window !== "undefined" && window.electronAPI?.whatsapp) {
            const whatsapp = window.electronAPI.whatsapp;
            ...
            if (prepared?.prepared && prepared.to && prepared.message) {
              const sent = await whatsapp.send({
                ...
                token: prepared.sendToken,
                expiresAt: prepared.expiresAt,
              });
              if (sent.ok) toast.success("Notifikasi WhatsApp owner terkirim");
              else
                toast.warning(
                  `WhatsApp owner tidak terkirim: ${sent.error || "WhatsApp belum siap"}`,
                );
            } else if (
              prepared?.reason &&
              !["disabled", "not_required"].includes(prepared.reason)
            ) {
              toast.warning(
                `WhatsApp owner tidak terkirim: ${prepared.reason}`,
              );
            }
            return;
          }
          ...
          if (data?.sent) toast.success("Notifikasi WhatsApp owner terkirim");
            ...
            data?.reason &&
            !["disabled", "not_required"].includes(data.reason)
          ) {
            toast.warning(
              `WhatsApp owner tidak terkirim: ${data.error || data.reason}`,
            );
          ...
          toast.warning(
            "WhatsApp owner tidak terkirim. Cek koneksi WhatsApp di Pengaturan.",
          );
        }
      })();
      closeModal();
  ...
  const favoriteSvcObjects = favorites
    .map((id) => services.find((s) => s.id === id))
    .filter(Boolean) as Service[];
  const selectedBankIsBankIcon = selectedBank
    ? isSupportedBankIcon(selectedBank.icon)
    : false;
  const formattedPhysicalCashAmount = formatRupiah(physicalCashAmount);
  const primaryActionLabel = flowConfig
    ? flowConfig.primaryActionText.replace(
        "{amount}",
        formattedPhysicalCashAmount,
      )
    : "";
    ...
    <div className="space-y-5 animate-fadeIn">
      <div>
        ...
        <p className="text-sm text-slate-400">
          Pilih layanan dan catat transaksi nasabah
        </p>
      </div>
      ...
      <div className="rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800">
        ...
        <p className="text-xs leading-relaxed">
          Aplikasi ini tidak mengirim transaksi ke bank/provider. Pastikan
          transaksi aktual dilakukan melalui kanal resmi, lalu catat hasilnya di
          sini.
        </p>
      </div>
          ...
          <div className="p-5 space-y-4">
            ...
            {step === "input" && (
              <div className="space-y-4">
                  ...
                  <div>
                    ...
                    {sel.useTieredFee && form.amount && currentTier && (
                      <p className="text-xs text-purple-600 bg-purple-50 px-2 py-1 rounded mt-1">
                        Tier: {formatRupiah(currentTier.minAmount)} -{" "}
                        {currentTier.maxAmount
                          ? formatRupiah(currentTier.maxAmount)
                          : "∞"}
                      </p>
                    )}
                  </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  />
                  <Input
                    label="No. HP / Rekening Tujuan"
                    value={form.customerPhone}
                    onChange={(e) =>
                      setForm({ ...form, customerPhone: e.target.value })
                    }
                    placeholder="08xx / 0012xxx"
                  />
                </div>
                    ...
                    {!showAccountPicker ? (
                      ...
                      >
                        <div className="flex items-center gap-2">
                          {selectedBank && selectedBankIsBankIcon && (
                            <BankIcon name={selectedBank.icon} size={18} />
                          )}
                          {selectedBank && !selectedBankIsBankIcon && (
                            <DynamicIcon
                              name={selectedBank.icon}
                              fallback="credit-card"
                              size={16}
                          ...
                          <div className="text-left">
                            <p className="text-sm font-semibold text-slate-700">
                              {selectedBank?.name || "Pilih rekening"}
                            </p>
                            <p className="text-xs text-slate-400">
                              Saldo:{" "}
                              {formatRupiah(selectedBank?.balance || "0")}
                            </p>
                          </div>
                        </div>
                      ...
                      <div className="space-y-2">
                        {bankAccounts.map((acc) => {
                          const isSelected =
                            form.selectedBankId === acc.id.toString();
                          const needsBalance = sel.bankEffect === "out";
                          const hasBalance =
                            parseFloat(acc.balance) >= bankImpactAmount;
                          return (
                            <button
                              key={acc.id}
                              onClick={() => {
                                setForm({
                                  ...form,
                                  selectedBankId: acc.id.toString(),
                                });
                                setShowAccountPicker(false);
                              }}
                              className={cn(
                  ...
                  cashAccount && (
                    <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 text-sm">
                      ...
                      <p className="text-amber-600 mt-1">
                        Saldo:{" "}
                        <strong>{formatRupiah(cashAccount.balance)}</strong>
                        {cashShortfall > 0 && (
                          <span className="text-red-600 ml-2">
                            — kurang {formatRupiah(cashShortfall)}
                          </span>
                        )}
                      </p>
                    </div>
                  ...
                  <div>
                    ...
                    >
                      <span className="flex items-center gap-2">
                        <Banknote size={14} /> Catat denominasi uang (opsional)
                      </span>
                      {showDenomination ? (
                        <ChevronUp size={14} />
                      ) : (
                      ...
                      >
                        ...
                        <div className="flex justify-between text-sm pt-2 border-t border-slate-200">
                          ...
                          <span
                            className={cn(
                              "font-bold",
                              denominationTotal === nominalAmount
                                ? "text-emerald-600"
                                : "text-amber-600",
                            )}
                          >
                            ...
                            {denominationTotal > 0 &&
                              denominationTotal !== nominalAmount && (
                                <span className="text-[10px] ml-1">
                                  (selisih{" "}
                                  {formatRupiah(
                                    Math.abs(denominationTotal - nominalAmount),
                                  )}
                                  )
                                </span>
                              )}
                  ...
                  <div className="bg-slate-50 rounded-xl p-3 flex items-center justify-between">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs font-medium text-slate-600">
                        {FEE_METHOD_LABELS[form.feeMethod]}
                      </p>
                    </div>
                  </div>
                )}
                ...
                {cashShortfall > 0 && (
                  <div className="bg-red-50 border border-red-200 rounded-xl p-3 text-xs text-red-700 flex items-center gap-2">
                    ...
                    <span>
                      Kas kurang {formatRupiah(cashShortfall)}. Pilih sumber
                      saldo lain atau batalkan.
                    </span>
                  </div>
                )}
            ...
            {step === "review" && (
              <div className="space-y-4">
                ...
                <Card
                  className={cn(
                    "p-4 bg-gradient-to-br border space-y-2",
                    toneClasses.summaryBg,
                    toneClasses.summaryBorder,
                  )}
                >
                  ...
                  <div className="border-t border-slate-200 pt-2 flex justify-between text-lg font-extrabold">
                    ...
                    <span className={toneClasses.accent}>
                      {formatRupiah(physicalCashAmount)}
                    </span>
                  </div>
                </Card>
                ...
                <div className="rounded-2xl border-2 border-slate-200 bg-white p-4 shadow-sm space-y-3">
                  <div className="flex items-center justify-between">
                    <div>
                      ...
                      <p className="text-sm font-semibold text-slate-700">
                        Perubahan saldo setelah transaksi dicatat
                      </p>
                    </div>
                    {agentFee > 0 && (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      >
                        ...
                        {formatRupiah(Math.abs(totalCashFlow))}
                      </p>
                      <p className="text-[11px] text-slate-400 mt-1">
                        {formatRupiah(cashBalanceBefore)} →{" "}
                        ...
                        >
                          {bankFlow.bankDelta > 0
                            ...
                            : bankFlow.bankDelta < 0
                              ? "−"
                              : ""}
                          {formatRupiah(bankImpactAmount)}
                        </p>
                        <p className="text-[11px] text-slate-400 mt-1">
                          {formatRupiah(bankBalanceBefore)} →{" "}
                          {formatRupiah(bankBalanceAfter)}
                        </p>
                      </div>
                    )}
                  </div>
                  ...
                  {showDetails && (
                    <div className="space-y-2 pt-2 border-t border-slate-100">
                      ...
                      {sel.bankEffect !== "none" && selectedBank && (
                        <div className="flex items-center justify-between p-2 rounded-lg bg-slate-50">
                          <span className="text-xs text-slate-500">
                            {selectedBank.name} sebelum → sesudah
                          </span>
                          <span className="text-xs font-medium">
                            {formatRupiah(bankBalanceBefore)} →{" "}
                            <span
            ...
            {step === "confirm" && (
              <div className="space-y-4 text-center">
                ...
                <div>
                  <h3 className="text-lg font-extrabold text-slate-800">
                    {flowConfig.confirmationHeading}
                  </h3>
                ...
                <div className="bg-slate-50 rounded-xl p-3 space-y-1.5 text-sm">
                  ...
                  <div className="flex justify-between">
                    <span className="text-slate-500">
                      Kas setelah transaksi
                    </span>
                    <span
                      className={cn(
                        "font-bold",
                        totalCashFlow < 0 ? "text-red-600" : "text-emerald-600",
                      )}
                    >
                      {formatRupiah(cashBalanceAfter)}
                    </span>
                ...
                {flowConfig.requiresCashHandling && (
                  <label className="flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-left cursor-pointer">
                    ...
                    <span className="text-sm font-semibold text-amber-800">
                      Saya sudah menghitung dan memastikan uang fisik sesuai
                      transaksi ini.
                    </span>
                  </label>
                )}
                ...
                <div className="flex gap-3 pt-2">
                  ...
                  <Button
                    ...
                    disabled={
                      submitting ||
                      (flowConfig.requiresCashHandling && !cashConfirmed)
                    }
                  >
                    {submitting
                      ? "Memproses..."
                      : flowConfig.confirmationConfirmText}
                  </Button>
                </div>
              </div>
            )}
