          ...
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="text-slate-400 text-[11px] font-medium">
                {settings.business_type || "Agen Bisnis"}
              </p>
            </div>
          </div>
          <button
            onClick={() => setOpen(false)}
            className="lg:hidden text-slate-400 hover:text-white active:scale-90 transition-all"
          >
            <X size={22} />
          </button>
        </div>
        ...
        <nav className="relative flex-1 px-3 py-2 space-y-1 overflow-y-auto">
          {navItems
            ...
            .map((item) => {
              ...
              const label =
                item.label === "__SERVICES__" ? servicesLabel : item.label;
