  ...
  return (
    <div className="space-y-5 animate-fadeIn">
      <div>
        ...
        <p className="text-sm text-gray-400">{trxs.length} transaksi ditemukan</p>
      </div>

      <div className="grid grid-cols-3 gap-3">
        <StatCard icon={<ShoppingCart size={18} />} label="Total" value={trxs.length.toString()} sub="transaksi" color="bg-blue-50 text-blue-500" />
        ...
        {loading ? <Spinner /> : trxs.length === 0 ? <EmptyState icon="📋" title="Belum ada transaksi" /> : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {trxs.map(t => (
                  <tr key={t.id} className="border-t border-gray-50 hover:bg-blue-50/30 transition-colors">
                    ...
                    <td className="p-3 text-gray-500 text-xs">{t.subType || "Penjualan"}</td>
        ...
        {loadingDet ? <Spinner /> : detail && (
          <div className="p-6 space-y-4">
            ...
            <div className="text-center bg-gray-50 rounded-xl p-4">
              ...
              <p className="text-xs text-gray-400 mt-1">{formatDate(detail.createdAt)}</p>
            </div>
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div><span className="text-gray-400">Tipe</span><p className="font-medium">{detail.type === "pos" ? "POS" : "BRILink"}</p></div>
              {detail.subType && <div><span className="text-gray-400">Layanan</span><p className="font-medium">{detail.subType}</p></div>}
              {detail.customerName && <div><span className="text-gray-400">Pelanggan</span><p className="font-medium">{detail.customerName}</p></div>}
              {detail.customerPhone && <div><span className="text-gray-400">No. HP/Rek</span><p className="font-medium">{detail.customerPhone}</p></div>}
              <div><span className="text-gray-400">Pembayaran</span><p className="font-medium capitalize">{detail.paymentMethod}</p></div>
            </div>
            {detail.items?.length > 0 && (
              <div className="border-t border-dashed pt-3 space-y-2">
                <p className="text-sm font-semibold text-gray-600">Item:</p>
                {detail.items.map(i => (
                  <div key={i.id} className="flex justify-between text-sm bg-gray-50 rounded-lg p-2">
                    <span>{i.productName} <span className="text-gray-400">x{i.quantity}</span></span>
            ...
            <div className="border-t border-dashed pt-3 space-y-1">
              {detail.adminFee && parseFloat(detail.adminFee) > 0 && (
                <div className="flex justify-between text-sm"><span className="text-gray-400">Biaya Admin</span><span className="text-amber-600 font-semibold">{formatRupiah(detail.adminFee)}</span></div>
              )}
