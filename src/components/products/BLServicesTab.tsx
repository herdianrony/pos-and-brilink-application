...
export default function BLServicesTab() {
  ...
  const [f, setF] = useState({
    ...
    adminFee: "",
    agentFee: "",
    profitDifferent: false,
    useTieredFee: false,
    cashEffect: "in",
    bankEffect: "out",
    flowType: "payment",
    defaultFeeMethod: "cash",
    presetId: "custom" as ServicePresetId,
    description: "",
  });
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [svcRes, catRes] = await Promise.all([
        fetch("/api/brilink-services"),
        fetch("/api/service-categories"),
      ]);
      if (!svcRes.ok || !catRes.ok)
        throw new Error("Gagal memuat layanan/kategori");
      const [s, c] = await Promise.all([svcRes.json(), catRes.json()]);
      setSvcs(s);
      setCats(c);
    } catch (error) {
      toast.error(
        error instanceof Error ? error.message : "Gagal memuat layanan",
      );
      setSvcs([]);
      setCats([]);
    } finally {
  ...
  function openEdit(s: BLService) {
    ...
    setF({
      ...
      icon: s.icon || "credit-card",
      adminFee: s.adminFee,
      agentFee: s.agentFee,
      profitDifferent: Number(s.adminFee) !== Number(s.agentFee),
      useTieredFee: s.useTieredFee,
      cashEffect: s.cashEffect || "in",
      bankEffect: s.bankEffect || "out",
      flowType: (s.flowType || "payment") as FlowType,
      defaultFeeMethod: (s.defaultFeeMethod || "cash") as FeeMethod,
      presetId: "custom",
  ...
  const simulationAgentFee = f.profitDifferent
    ? Number(f.agentFee || 0)
    : simulationAdminFee;
  const simulationCash = calculateCashFlow(
    f.cashEffect,
    simulationNominal,
    simulationAdminFee,
    f.defaultFeeMethod as FeeMethod,
  );
  const simulationBank = calculateBankFlow(
    f.cashEffect,
    f.bankEffect,
    simulationNominal,
    simulationAdminFee,
  ...
  async function save() {
    ...
    setSaving(true);
    try {
      const agentFee = f.profitDifferent
        ? f.agentFee || "0"
        : f.adminFee || "0";
      const selectedCategory = cats.find(
        (cat) => String(cat.id) === f.categoryId,
      );
      const res = await fetch("/api/brilink-services", {
      ...
      await load();
      toast.success(
        edit ? "Layanan berhasil diupdate" : "Layanan berhasil ditambahkan",
      );
    } catch (error) {
      toast.error(
  ...
  async function saveTiers() {
    ...
    setSaving(true);
    try {
      const svcRes = await fetch("/api/brilink-services", {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          id: edit.id,
      ...
      if (!svcRes.ok)
        throw new Error(svcBody.error || "Gagal menyimpan fee berjenjang");

      setTiersModal(false);
      await load();
      toast.success("Fee berjenjang berhasil disimpan");
    } catch (error) {
      toast.error(
        error instanceof Error
          ? error.message
          : "Gagal menyimpan fee berjenjang",
      );
    } finally {
      setSaving(false);
    }
  ...
  async function del(id: number) {
    try {
      ...
      if (!res.ok) throw new Error(body.error || "Gagal menghapus layanan");
      await load();
      ...
      <Card className="overflow-hidden">
        {loading ? (
          ...
          <div className="p-8 text-center space-y-3">
            <EmptyState
              icon="landmark"
              title="Belum ada layanan agen"
              subtitle="Buat layanan pertama sesuai SOP usaha Anda: Tarik Tunai, Setor Tunai, Transfer, Pembayaran, Top Up, atau layanan custom."
            />
            <Button onClick={openAdd}>
              <Plus size={16} /> Buat Layanan Pertama
            </Button>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="p-3 text-center">
                      <div className="flex items-center justify-center gap-1">
                        <button
                          onClick={() => openTiers(s)}
                          className="p-1.5 text-purple-500 hover:bg-purple-50 rounded-xl"
                          title="Atur Fee Berjenjang"
                        >
                          <Layers size={14} />
                        </button>
                        <button
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Pilih preset agar efek kas/rekening otomatis benar. Anda masih
                bisa menyesuaikan setelahnya.
              </p>
            </div>
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              ...
              <div>
                ...
                <p className="text-xs text-purple-600">
                  Fee otomatis berubah berdasarkan nominal transaksi
                </p>
              </div>
            </label>
          </div>
            ...
            <div className="space-y-3">
              <CurrencyInput
                ...
                onChange={(value) =>
                  setF({
                    ...f,
                    adminFee: String(value),
                    agentFee: f.profitDifferent ? f.agentFee : String(value),
                  })
                }
                placeholder="0"
              />
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input
                    ...
                    onChange={(e) =>
                      setF({
                        ...f,
                        profitDifferent: e.target.checked,
                        agentFee: e.target.checked ? f.agentFee : f.adminFee,
                      })
                    }
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400">
                      Default: profit agen otomatis sama dengan biaya admin.
                    </p>
                  </div>
                </label>
                {f.profitDifferent && (
              ...
              {tiers.length === 0 ? (
                ...
                <div className="space-y-2">
                  {tiers.map((tier, idx) => (
                    <div
                      key={idx}
                      className="flex items-end gap-2 bg-slate-50 rounded-xl p-3"
                    >
                      <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                        <CurrencyInput
                          ...
                          onChange={(value) =>
                            updateTier(idx, "minAmount", String(value))
                          }
                          placeholder="0"
                        />
                        <CurrencyInput
                        ...
                        <CurrencyInput
                          ...
                          value={tier.adminFee}
                          onChange={(value) =>
                            updateTier(idx, "adminFee", String(value))
                          }
                          placeholder="0"
                        />
                      </div>
                  ...
                  <p className="text-xs text-slate-400">
                    Profit agen pada tier otomatis mengikuti biaya admin.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <Select
              label="Kas Tunai"
              value={f.cashEffect}
              onChange={(e) => setF({ ...f, cashEffect: e.target.value })}
            >
            ...
            <Select
              label="Rekening / M-Banking"
              value={f.bankEffect}
              onChange={(e) => setF({ ...f, bankEffect: e.target.value })}
            >
              <option value="in">Rekening masuk / terima transfer</option>
              <option value="out">
                Rekening keluar / transfer atau bayar provider
              </option>
            ...
            <Select
              label="Jenis Flow"
              value={f.flowType}
              onChange={(e) => setF({ ...f, flowType: e.target.value })}
            >
            ...
            <Select
              label="Cara Admin Dibayar"
              value={f.defaultFeeMethod}
              onChange={(e) => setF({ ...f, defaultFeeMethod: e.target.value })}
            >
              <option value="cash">Dibayar cash oleh pelanggan</option>
              <option value="charged">
                Ditambahkan ke transfer/saldo pelanggan
              </option>
              <option value="deducted">
                Dipotong dari nominal yang diterima pelanggan
              </option>
            </Select>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white p-4 space-y-3">
            <div>
              ...
              <p className="text-xs text-slate-400">
                Contoh nominal {formatRupiah(simulationNominal)} dengan admin{" "}
                {formatRupiah(simulationAdminFee || 0)}.
              </p>
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                ...
                <p className="text-lg font-extrabold">
                  {simulationCash.cashDelta > 0
                    ...
                    : simulationCash.cashDelta < 0
                      ? "−"
                      : ""}
                  {formatRupiah(Math.abs(simulationCash.cashDelta))}
                </p>
              </div>
              <div
                className={cn(
                  "rounded-xl p-3",
                  simulationBank.bankDelta < 0
                    ? "bg-red-50 text-red-700"
                    : simulationBank.bankDelta > 0
                      ? "bg-emerald-50 text-emerald-700"
                      : "bg-slate-50 text-slate-600",
                ...
                <p className="text-lg font-extrabold">
                  {simulationBank.bankDelta > 0
                    ...
                    : simulationBank.bankDelta < 0
                      ? "−"
                      : ""}
                  {formatRupiah(Math.abs(simulationBank.bankDelta))}
                </p>
              </div>
              <div className="rounded-xl bg-emerald-50 p-3 text-emerald-700">
                ...
                <p className="text-lg font-extrabold">
                  +{formatRupiah(Math.max(0, simulationAgentFee))}
                </p>
              </div>
            </div>
          </div>
          <div className="bg-emerald-50 border border-emerald-100 rounded-xl p-3 text-xs text-emerald-700">
            ...
            <ul className="list-disc ml-4 space-y-0.5">
              <li>
                <strong>Tarik Tunai:</strong> Cash minus keluar, M-Banking plus
                masuk
              </li>
              <li>
                <strong>Transfer/Bayar/Setor:</strong> Cash plus masuk,
                M-Banking minus keluar
              </li>
            </ul>
          </div>
          <div>
            ...
            <div className="flex flex-wrap gap-2 max-h-24 overflow-y-auto bg-slate-50 rounded-xl p-3">
              {icons.slice(30).map((em) => (
                ...
                >
                  <DynamicIcon name={em} size={18} className="text-slate-700" />
                </button>
              ))}
            </div>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <Button variant="primary" size="sm" onClick={addTier}>
                <Plus size={14} /> Tambah Tier
              </Button>
            </div>
            ...
            {tiers.length === 0 ? (
              <div className="text-center py-8 bg-slate-50 rounded-xl">
                ...
                <p className="text-gray-300 text-xs">
                  Klik "Tambah Tier" untuk membuat skema fee berjenjang
                </p>
              </div>
            ) : (
              <div className="space-y-2">
                ...
                {tiers.map((tier, idx) => (
                  <div
                    key={idx}
                    className="flex items-end gap-2 bg-slate-50 rounded-xl p-3 border border-slate-100"
                  >
                    <div className="flex-1 grid grid-cols-1 sm:grid-cols-3 gap-2">
                      <CurrencyInput
                        label="Min"
