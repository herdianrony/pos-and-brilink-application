    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-zinc-400">Ringkasan aktivitas bisnis Anda</p>
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-semibold text-zinc-500 uppercase tracking-wider">Saldo Rekening</h3>
          <span className="text-xs text-zinc-400">{d.accounts.length} akun aktif</span>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="p-5 relative overflow-hidden group">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-end">
              <div>
                <p className="text-3xl font-bold text-zinc-900">{d.today.pos.count}</p>
        ...
        <Card className="p-5 relative overflow-hidden group">
          ...
          <div className="relative">
            ...
            <div className="flex justify-between items-end">
              <div>
                <p className="text-3xl font-bold text-zinc-900">{d.today.brilink.count}</p>
                <p className="text-xs text-zinc-400">transaksi</p>
              </div>
              <div className="text-right">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
              ...
              {d.last7.map((day, i) => {
                ...
                return (
                  <div key={i} className="flex-1 flex flex-col items-center gap-1.5 group">
                    <span className="text-[10px] text-zinc-500 opacity-0 group-hover:opacity-100 transition-opacity font-medium">
                    ...
                    <span className="text-[10px] text-zinc-400 font-medium">{formatDateShort(day.date)}</span>
                  </div>
                );
              })}
            </div>
        ...
        <Card className="p-5">
          ...
          {d.lowStock.length === 0 ? (
            <div className="text-center py-6 text-sm text-zinc-400 flex flex-col items-center gap-2">
            ...
            <div className="space-y-2 max-h-44 overflow-y-auto">
              {d.lowStock.map(p => (
                <div key={p.id} className="flex items-center justify-between p-2.5 bg-amber-50/50 rounded-xl border border-amber-100">
                  <span className="text-sm text-zinc-700 truncate flex-1">{p.name}</span>
      ...
      <Card className="overflow-hidden">
        <div className="p-5 border-b border-zinc-100">
          <h3 className="font-semibold text-zinc-700">Transaksi Terakhir</h3>
        </div>
        {d.recent.length === 0 ? (
              ...
              <tbody>
                {d.recent.map(t => (
                  <tr key={t.id} className="border-b border-zinc-50/50 hover:bg-indigo-50/30 transition-colors">
                    ...
                    <td className="p-3 text-zinc-400 text-xs">{formatDate(t.createdAt)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
