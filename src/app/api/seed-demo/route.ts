...
export async function POST() {
  ...
  try {
    ...
    const result = await runTransaction(async (tx) => {
      ...
      const existingDemoCats = await tx
        .select()
        .from(categories)
      ...
      if (existingDemoCats.length === 0) {
        cats = await tx
          ...
          .values([
            {
              name: `${DEMO_MARKER} Makanan`,
              icon: "utensils",
              color: "#ef4444",
            },
            {
              name: `${DEMO_MARKER} Minuman`,
              icon: "cup-soda",
              color: "#3b82f6",
            },
            { name: `${DEMO_MARKER} Rokok`, icon: "package", color: "#6b7280" },
            {
              name: `${DEMO_MARKER} Sembako`,
              icon: "shopping-cart",
              color: "#22c55e",
            },
            ...
            {
              name: `${DEMO_MARKER} Toiletries`,
              icon: "spray-can",
              color: "#ec4899",
            },
            {
              name: `${DEMO_MARKER} Aksesoris HP`,
              icon: "smartphone",
              color: "#06b6d4",
            },
            {
              name: `${DEMO_MARKER} Pulsa & Voucher`,
              icon: "smartphone",
              color: "#00875A",
            },
            {
              name: `${DEMO_MARKER} Gas & Listrik`,
              icon: "zap",
              color: "#f97316",
            },
          ])
      ...
      // ── E2E/dev demo balances ──
      // Production seed intentionally keeps balances empty/dynamic. For E2E/demo,
      // activate one bank account and set deterministic balances so BRILink flows
      // and insufficient-balance regressions are testable.
      if (process.env.NODE_ENV !== "production") {
        await tx
          .update(accounts)
          .set({ balance: 500_000, isActive: true })
          .where(eq(accounts.code, "cash"));
        await tx
          .update(accounts)
          .set({ balance: 2_000_000, isActive: true })
          .where(eq(accounts.code, "bank_bri"));
        stats.demoBalances = 2;
      }

      const cm: Record<string, number> = {};
      for (const c of cats) cm[c.name] = c.id;
      if (cats.length === 0) {
        const allDemoCats = await tx
          .select()
          .from(categories)
          .where(sql`${categories.name} LIKE ${DEMO_MARKER + "%"}`);
      ...
      const existingDemoProducts = await tx
        .select()
        .from(products)
      ...
      if (existingDemoProducts.length === 0) {
        await tx.insert(products).values([
          {
            name: `${DEMO_MARKER} Indomie Goreng`,
            barcode: "089686010947",
            categoryId: cm[`${DEMO_MARKER} Makanan`],
            buyPrice: 2500,
            sellPrice: 3500,
            stock: 120,
            minStock: 20,
          ...
          {
            name: `${DEMO_MARKER} Indomie Kuah Soto`,
            barcode: "089686010602",
            categoryId: cm[`${DEMO_MARKER} Makanan`],
            buyPrice: 2500,
            sellPrice: 3500,
            stock: 80,
          ...
          {
            name: `${DEMO_MARKER} Mie Sedaap Goreng`,
            barcode: "089686040203",
            categoryId: cm[`${DEMO_MARKER} Makanan`],
            buyPrice: 2800,
            sellPrice: 3500,
            stock: 60,
          ...
          {
            name: `${DEMO_MARKER} Aqua 600ml`,
            barcode: "761100100019",
            categoryId: cm[`${DEMO_MARKER} Minuman`],
            buyPrice: 2800,
            sellPrice: 4000,
            stock: 200,
            minStock: 30,
          ...
          {
            name: `${DEMO_MARKER} Teh Botol Sosro 450ml`,
            barcode: "899900100219",
            categoryId: cm[`${DEMO_MARKER} Minuman`],
            buyPrice: 3500,
            sellPrice: 5000,
            stock: 48,
          ...
          {
            name: `${DEMO_MARKER} Le Minerale 600ml`,
            barcode: "749100100012",
            categoryId: cm[`${DEMO_MARKER} Minuman`],
            buyPrice: 2200,
            sellPrice: 3500,
            stock: 150,
          ...
          {
            name: `${DEMO_MARKER} Coca Cola 390ml`,
            barcode: "545500200100",
            categoryId: cm[`${DEMO_MARKER} Minuman`],
            buyPrice: 5500,
            sellPrice: 7500,
            stock: 36,
          ...
          {
            name: `${DEMO_MARKER} GG Surya 12`,
            barcode: "899001010012",
            categoryId: cm[`${DEMO_MARKER} Rokok`],
            buyPrice: 20000,
            sellPrice: 24000,
            stock: 50,
            minStock: 10,
          ...
          {
            name: `${DEMO_MARKER} Sampoerna Mild 16`,
            barcode: "899001020016",
            categoryId: cm[`${DEMO_MARKER} Rokok`],
            buyPrice: 28000,
            sellPrice: 32000,
            stock: 40,
          ...
          {
            name: `${DEMO_MARKER} Djarum Super 12`,
            barcode: "899001030012",
            categoryId: cm[`${DEMO_MARKER} Rokok`],
            buyPrice: 19000,
            sellPrice: 22000,
            stock: 35,
          ...
          {
            name: `${DEMO_MARKER} Beras Premium 5kg`,
            categoryId: cm[`${DEMO_MARKER} Sembako`],
            buyPrice: 62000,
            sellPrice: 70000,
            stock: 25,
            minStock: 5,
      ...
      const existingSvcCats = await tx
        .select()
      ...
      if (existingServices.length === 0) {
        await tx.insert(brilinkServices).values([
          {
            code: "transfer_cash",
            name: "Kirim Transfer Tunai",
            categoryId: scm["transfer"],
          ...
          },
          {
            ...
            defaultFeeMethod: "deducted",
          },
          {
            code: "cash_withdrawal",
            name: "Tarik Tunai",
            categoryId: scm["cash_withdrawal"],
          ...
          {
            code: "voucher_game_12k",
            name: "Voucher Game 12K",
            categoryId: scm["voucher"],
          ...
          {
            code: "voucher_game_33k",
            name: "Voucher Game 33K",
            categoryId: scm["voucher"],
            categoryCode: "voucher",
            icon: "gift",
            adminFee: 1500,
          ...
          {
            code: "voucher_game_66k",
            name: "Voucher Game 66K",
            categoryId: scm["voucher"],
            categoryCode: "voucher",
            icon: "gift",
            adminFee: 2000,
          ...
          {
            code: "voucher_game_132k",
            name: "Voucher Game 132K",
            categoryId: scm["voucher"],
            categoryCode: "voucher",
            icon: "gift",
            adminFee: 2500,
          ...
          {
            code: "voucher_game_330k",
            name: "Voucher Game 330K",
            categoryId: scm["voucher"],
            categoryCode: "voucher",
            icon: "gift",
            adminFee: 3000,
          ...
          {
            code: "voucher_game_600k",
            name: "Voucher Game 600K",
            categoryId: scm["voucher"],
            categoryCode: "voucher",
            icon: "gift",
            adminFee: 5000,
          ...
          },
          {
            code: "financing_fif",
            name: "Cicilan FIF",
            categoryId: scm["financing"],
            categoryCode: "financing",
            icon: "file-text",
            adminFee: 2500,
            agentFee: 2500,
            useTieredFee: false,
      ...
      for (const svc of svcs) {
        ...
        const existingTiers = await tx
          .select()
          .from(feeTiers)
          .where(eq(feeTiers.serviceId, svc.id));
        ...
        if (svc.code === "cash_withdrawal") {
          ...
          const nominal =
            codePart === "1jt"
              ? 1000000
              : parseInt(codePart.replace("k", "000"));
          if (nominal > 0) {
            const fee =
              nominal >= 500000 ? 3000 : nominal >= 100000 ? 2000 : 1500;
            tiers = [
              {
                minAmount: nominal,
                maxAmount: nominal,
          ...
          const nominal = parseInt(
            svc.code.replace("voucher_game_", "").replace("k", "000"),
          );
          if (nominal > 0) {
            const fee =
              nominal >= 330000
        ...
        if (tiers.length > 0) {
          await tx
            .insert(feeTiers)
            .values(tiers.map((t) => ({ serviceId: svc.id, ...t })));
