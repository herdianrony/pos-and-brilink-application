...
const VALID_EFFECTS = new Set(["in", "out", "none"]);
...
const VALID_FEE_METHODS = new Set(["cash", "deducted", "charged"]);

function validateServiceFlowFields(body: Record<string, unknown>) {
  const cashEffect = String(body.cashEffect || "in");
  const bankEffect = String(body.bankEffect || "out");
  const flowType = String(body.flowType || "payment");
  const defaultFeeMethod = String(body.defaultFeeMethod || "cash");

  if (!VALID_EFFECTS.has(cashEffect)) throw new Error("cashEffect tidak valid");
  if (!VALID_EFFECTS.has(bankEffect)) throw new Error("bankEffect tidak valid");
  if (!VALID_FLOW_TYPES.has(flowType)) throw new Error("flowType tidak valid");
  if (!VALID_FEE_METHODS.has(defaultFeeMethod))
    throw new Error("defaultFeeMethod tidak valid");

  return { cashEffect, bankEffect, flowType, defaultFeeMethod };
}
...
function normalizeFeeTiers(rawTiers: unknown, serviceId: number) {
  ...
  const tiers = rawTiers
    .map((tier) => {
      ...
      const agentFee = parseSafeNumber(t.agentFee ?? t.adminFee, {
        min: 0,
        default: adminFee,
      });
  ...
  for (const tier of tiers) {
    if (tier.maxAmount !== null && tier.maxAmount < tier.minAmount) {
      throw new Error(
        `Tier tidak valid: nominal max ${tier.maxAmount} lebih kecil dari nominal min ${tier.minAmount}`,
      );
    }
    if (previousMax !== null && tier.minAmount <= previousMax) {
      throw new Error(
        "Tier fee berjenjang overlap. Pastikan nominal min lebih besar dari max tier sebelumnya.",
      );
    }
...
export async function GET() {
  try {
    ...
    const services = await db
      ...
      .leftJoin(
        serviceCategories,
        eq(brilinkServices.categoryId, serviceCategories.id),
      )
    ...
    const tiers = await db
      .select()
      .from(feeTiers)
      .orderBy(asc(feeTiers.serviceId), asc(feeTiers.minAmount));
    ...
    return handleApiError(
      "src/app/api/brilink-services/route.ts:GET",
      error,
      "Gagal memproses data layanan",
    );
  }
}
...
export async function POST(req: Request) {
  try {
    ...
    let flowFields;
    try {
      flowFields = validateServiceFlowFields(b);
    } catch (error) {
      return NextResponse.json(
        {
          error:
            error instanceof Error
              ? error.message
              : "Konfigurasi flow layanan tidak valid",
        },
        { status: 400 },
      );
    }
    ...
    return await runTransaction(async (tx) => {
      ...
      const [row] = await tx
        ...
        .values({
          ...
          cashEffect: flowFields.cashEffect,
          bankEffect: flowFields.bankEffect,
          flowType: flowFields.flowType,
          defaultFeeMethod: flowFields.defaultFeeMethod,
          description: b.description || null,
        })
        .returning();

      const tiers = b.useTieredFee
        ? normalizeFeeTiers(b.feeTiers || b.tiers, row.id)
        : [];
      if (tiers.length > 0) {
    ...
    return handleApiError(
      "src/app/api/brilink-services/route.ts:POST",
      error,
      "Gagal memproses data layanan",
    );
  }
...
export async function PUT(req: Request) {
  try {
    ...
    if (!auth.ok) return auth.response;
    const b = await req.json();
    const serviceId = parsePositiveId(b.id);
    if (!serviceId)
      return NextResponse.json(
    ...
    return await runTransaction(async (tx) => {
      const [row] = await tx
        ...
        .where(eq(brilinkServices.id, serviceId))
        .returning();

      await tx.delete(feeTiers).where(eq(feeTiers.serviceId, serviceId));
      const tiers = b.useTieredFee
        ? normalizeFeeTiers(b.feeTiers || b.tiers, serviceId)
        : [];
      if (tiers.length > 0) {
        await tx.insert(feeTiers).values(tiers);
      }
    ...
    return handleApiError(
      "src/app/api/brilink-services/route.ts:PUT",
      error,
      "Gagal memproses data layanan",
    );
  }
...
export async function DELETE(req: Request) {
  try {
    ...
    return handleApiError(
      "src/app/api/brilink-services/route.ts:DELETE",
      error,
      "Gagal memproses data layanan",
    );
  }
